    ...
    <div className="min-h-screen bg-light pb-24 overflow-hidden relative">
      ...
      {isOnline && (
        <motion.button
            whileTap={{ scale: 0.9 }}
            onClick={fetchOrders}
            className="fixed bottom-6 right-6 z-50 w-14 h-14 bg-black text-white rounded-full shadow-2xl flex items-center justify-center border-2 border-white/20"
        >
...
function OrderCard({ order, index, onUpdateStatus }) {
    ...
    const restCoords = order.restaurantId.address?.coordinates?.coordinates;
    const userCoords = order.deliveryAddress?.coordinates?.coordinates;

    // Google Maps Navigation URLs
    const storeMapLink = restCoords 
        ? `https://www.google.com/maps/dir/?api=1&destination=${restCoords[1]},${restCoords[0]}`
        : '#';
    
    const customerMapLink = userCoords
        ? `https://www.google.com/maps/dir/?api=1&destination=${userCoords[1]},${userCoords[0]}`
        : '#';
    ...
    return (
        ...
        >
            ...
            <div className="p-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                 <span className="text-xs font-black text-gray-400 tracking-wider">ORDER #{order.orderNumber.slice(-6)}</span>
                 <span className="text-xs font-bold bg-orange-100 text-orange-700 px-2 py-1 rounded-md uppercase">
            ...
            <div className="p-5 space-y-6">
                ...
                <div className="relative pl-6 border-l-2 border-gray-200">
                    ...
                    <div className="mb-1 text-[10px] font-bold text-gray-400 uppercase tracking-wider">Pickup From</div>
                    
                    <div className="flex justify-between items-start gap-2">
                        <div>
                            <h4 className="font-bold text-dark text-lg leading-tight">{order.restaurantId.restaurantName}</h4>
                            <p className="text-sm text-gray-500 mt-1 leading-snug">
                                {order.restaurantId.address?.addressLine1}, {order.restaurantId.address?.city}
                            </p>
                        </div>
                        <a 
                            href={storeMapLink}
                            target="_blank" rel="noreferrer"
                            className="w-10 h-10 bg-gray-100 text-dark rounded-xl flex items-center justify-center shrink-0 hover:bg-gray-200 transition"
                        >
                            <Navigation size={18} />
                        </a>
                    </div>
                ...
                <div className="relative pl-6 border-l-2 border-dashed border-gray-200">
                    ...
                    <div className="mb-1 text-[10px] font-bold text-primary uppercase tracking-wider">Deliver To</div>
                    
                    <div className="flex justify-between items-start gap-2">
                        <div>
                            <h4 className="font-bold text-dark text-lg leading-tight">{order.customerDetails?.name}</h4>
                            <div className="flex items-center gap-2 text-sm text-gray-600 font-bold mt-0.5">
                                <Phone size={12} className="text-gray-400" />
                                {order.customerDetails?.phoneNumber}
                            </div>
                            <p className="text-sm text-gray-500 mt-1 leading-snug">
                                {order.deliveryAddress?.addressLine1 || "Full address not provided"}
                                {order.deliveryAddress?.landmark && <span className="block text-xs text-gray-400 mt-0.5">(Landmark: {order.deliveryAddress.landmark})</span>}
                            </p>
                        </div>
                        <div className="flex flex-col gap-2 shrink-0">
                            <a 
                                ...
                                target="_blank" rel="noreferrer"
                                className="w-10 h-10 bg-primary/10 text-primary rounded-xl flex items-center justify-center hover:bg-primary/20 transition"
                            >
                                <Navigation size={18} />
                            </a>
                            <a 
                                href={`tel:${order.customerDetails?.phoneNumber}`}
                                className="w-10 h-10 bg-green-500 text-white rounded-xl flex items-center justify-center shadow-lg shadow-green-200 hover:bg-green-600 transition"
                            >
                ...
                <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                    <div className="flex items-center gap-2 text-xs font-bold text-gray-400 uppercase mb-3">
                        <FileText size={14} /> Order Contents
                    </div>
                            ...
                            <div key={idx} className="flex justify-between text-sm">
                                <div className="flex gap-3">
                                    ...
                                    <div className="text-gray-700">
                                        ...
                                        {/* Variants/Addons info if available */}
                    ...
                    {order.notes && (
                        <div className="mt-3 pt-3 border-t border-gray-200/50">
                            <p className="text-xs text-gray-500 italic flex gap-1">
                                <AlertCircle size={12} className="mt-0.5" /> Note: "{order.notes}"
                            </p>
                        </div>
                    )}
                    ...
                    <div className="mt-4 pt-3 border-t border-gray-200 flex justify-between items-center">
                        <div className="text-xs text-gray-500 font-medium bg-white px-2 py-1 rounded border border-gray-200">
                            {order.paymentType === 'cash' ? 'Cash on Delivery' : 'Paid Online'}
                        </div>
                        <div className="text-lg font-black text-dark">£{order.pricing?.totalAmount}</div>
                    </div>
                </div>
                ...
                {order.status === 'out_for_delivery' && (
                    <div className="pt-2">
                        {order.paymentType === 'cash' && order.paymentStatus === 'pending' && (
                            <div className="mb-3 p-3 bg-red-50 text-red-700 text-sm font-bold text-center rounded-xl border border-red-100 flex items-center justify-center gap-2">
                                <DollarSign size={16} /> Collect £{order.pricing?.totalAmount} Cash
                            </div>
                        )}
